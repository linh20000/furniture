...
class InteriorProductController extends Controller

{
    public function search(Request $request){
        // dd($request->all());
        $product = ProductInterior::where('name', 'LIKE', '%'. $request->search .'%')->orWhere('product_code', 'LIKE', '%'. $request->search .'%')->get();
        $productLenght = count($product);
        return view('backend.product_interior.list',['breadcrumb'=>'Danh sách sản phẩm'],compact('product','productLenght'));
    }
    public function showProductList() {
        $product = ProductInterior::all();
        // dd($dataProduct);
        $productLenght = count($product);
        return view('backend.product_interior.list',['breadcrumb'=>'Danh sách sản phẩm'],compact('product','productLenght'));
    }
    public function getCreateProduct() {
        // $category_id = Product_interior::where('category_id','=', '1')->where('status','=','0')->get();
        $category_id = Interior::where('parent_id' ,'!=' , 0)->where('status', '=', 1)->get();
        return view('backend.product_interior.create',['breadcrumb'=>'Thêm sản phẩm'],compact('category_id'));
    }
    public function addProduct( Request $request) {
        ...
        ProductInterior::create($data);
        return back()->with('success', 'Thêm thành công sản phẩm !');
    }
    
    public function getUpdateProduct($id) {
        $product = ProductInterior::find($id);
        $category_id = interior::where('parent_id' ,'=' , 0) ->get();
        return view('backend.product_interior.update',['breadcrumb'=>'Chỉnh sửa sản phẩm'], compact('product','category_id'));
    }

    public function updateProduct($id, Request $request) {
        $data_update = $request->all();
        // dd($data);
        $product_update = ProductInterior::findOrFail($id);
        $product_update -> update($data_update);
        return redirect(route('admin.InteriỏshowProductList'))->with('success', 'Chỉnh sửa thành công'); 
    }
    

    public function deleteProduct($id) {
        $product_delete = ProductInterior::find($id) ;
