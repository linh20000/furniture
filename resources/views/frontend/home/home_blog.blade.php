        ...
        <div class="home-blog-wrap">
            <div class="home-blog-left">
                <div class="section-title-all">
                    <h2>{{$blog_ichi->name}}</h2>
                    <p>Cùng tìm hiểu kiến thức về {{$blog_ichi->name}}</p>
                </div>
                <div class="home-blog-data">
                    <article class="article-item " data-index="1">
                        <div class="article-item-wrap">
                            <div class="article-item-image">
                                <a href="{{route('blogDetail', [$blog_ichi->id, Str::slug($blog_ichi->name)])}}" title="{{$blog_ichi->name}}" rel="nofollow">
                                    
                                    <img src="{{$blog_ichi->image}}"
                                            class="lazyload"
                                            data-src="{{$blog_ichi->image}}" 
                                            alt="{{$blog_ichi->name}}"> 
                            ...
                            <div class="article-item-detail">
                                <h3 class="article-item-detail-name"><a href="blogs/news/kinh-nghiem-lua-chon-noi-that-chung-cu.html" title="Kinh nghiệm lựa chọn nội thất chung cư">Kinh nghiệm lựa chọn nội thất chung cư</a></h3>
                                <strong class="article-item-detail-info">{{$blog_ichi->created_at->format('d')}} Thg {{$blog_ichi->created_at->format('m Y')}} / {{$blog_ichi->subname}}</strong>
