...
F1GENZ.Product = {
  init: function () {
    ...
    if ($(".main-product-seen").length > 0) {
      F1GENZ.Helper.updateWCS(
        ...
        window.F1GENZ_vars.product.data.handle, // Handle
    ...
    try {
      if (window.F1GENZ_vars.shop.featured.super_fs.all_day) {
        ...
        $(".main-product-fsale [data-start][data-end]").each(function () {
          // Start
          var init_timeS = moment(
            F1GENZ.Helper.formatTime($(this).attr("data-start"))
          ).format("HH:mm:ss");
          var finish_dateS = `${now_date} ${init_timeS}`;
          $(this).attr("data-start", finish_dateS);

          // End
          var init_timeE = moment(
            F1GENZ.Helper.formatTime($(this).attr("data-end"))
          ).format("HH:mm:ss");
      ...
      $.each(self.statusVariants, function (keyC2, valC2) {
        if (String(valC2.val).indexOf(valC1) !== -1) {
          if (valC2.status === true) {
            $(
              '.main-product .product-sw-select-item input[value="' +
                valC1 +
                '"]'
            ).removeClass("soldOut");
            if (flagClick0 == false) {
    ...
    if (name.indexOf(1) !== -1) {
      ...
      if (window.F1GENZ_vars.product.data.options.length == 3) {
        ...
        $(".main-product .product-sw-line")
          .eq(2)
          .find("input")
          .each(function (key23, val3) {
            var flagOption2 = $(this).val();
            var flagCheck = `${$(".main-product .product-sw-line")
              .eq(0)
              .find("input:checked")
              .val()},${value},${flagOption2}`;
            $.each(self.statusVariants, function (keyC, valC) {
              if (String(valC.val) === String(flagCheck) && valC.status) {
                $(
                  '.main-product .product-sw-select-item input[value="' +
  ...
  changeOption: function () {
    ...
    $("body").on("change", ".main-product .trigger-option-sw", function (e) {
      ...
      var value = $(this).val();
      $(
        ".main-product select[data-option=" +
          name +
  ...
  render: function () {
    if (navigator.userAgent.indexOf("Chrome-Lighthouse") === -1) {
      new Haravan.OptionSelectors("main-product-select", {
        product: window.F1GENZ_vars.product.data,
  ...
  variants: function (variant, selector) {
    if (variant) {
      // Change Image
      var items = $(
        '.main-product .main-product-feature .main-product-feature-thumbs a[data-id="' +
          variant.featured_image.id +
          '"]'
      );
      if (window.F1GENZ_vars.product.featured.style_gallery == "style3") {
        $("body, html").animate(
          { scrollTop: items.offset().top - 10 },
          500,
          "swing"
        );
      } else {
        if (
          $(
            ".main-product .main-product-feature .main-product-feature-thumbs.slick-slider"
          ).length > 0
        ) {
          $(
            ".main-product .main-product-feature .main-product-feature-thumbs"
          ).slick("slickGoTo", parseInt(items.attr("data-index")) - 1);
        }
      }
      // End Change Image

      window.F1GENZ_vars.product.availableOption = variant.available;
      var saleChange = Math.round(
        100 - variant.price / (variant.compare_at_price / 100)
      );
      if (variant.price == 0) {
        $(".main-product .main-product-price .main-product-price-compare").prop(
          "hidden",
          true
        );
        $(
          ".main-product .main-product-price .main-product-price-discount"
        ).prop("hidden", true);
        $(".main-product .main-product-price .main-product-price-this")
          .addClass("contact")
          .html(
        ...
        $(".main-product .main-product-price .main-product-price-this")
          .removeClass("contact")
          .text(
            Haravan.formatMoney(variant.price, window.F1GENZ_vars.formatMoney)
          );
        if (variant.compare_at_price > variant.price) {
          $(".main-product .main-product-price .main-product-price-compare")
            .text(
              Haravan.formatMoney(
                variant.compare_at_price,
                window.F1GENZ_vars.formatMoney
              )
            )
            .prop("hidden", false);
          $(".main-product .main-product-price .main-product-price-discount")
            .html("Tiết kiệm " + saleChange + "%")
            .prop("hidden", false);
        } else {
          $(
            ".main-product .main-product-price .main-product-price-compare"
      ...
      if (variant.sku != null) {
        $(".main-product .main-product-info .main-product-info-sku span").html(
          variant.sku
        );
      } else {
        $(".main-product .main-product-info .main-product-info-sku span").html(
      ...
      if (variant.barcode != null) {
        $(
          ".main-product .main-product-info .main-product-info-barcode span"
      ...
      $.each(window.F1GENZ_vars.product.data.variants, function (i, v) {
        if (v.available) {
          if (v.option1 && v.option1.length > 0) {
            if (
              $('.main-product .trigger-option-sw[data-name="option1"]').is(
                "input"
              )
            ) {
              $(
                '.main-product .trigger-option-sw[data-name="option1"][value="' +
                  v.option1 +
                  '"]'
              ).trigger("click");
            } else {
              $('.main-product .trigger-option-sw[data-name="option1"]')
                .val(v.option1)
                .trigger("change");
            }
          }
          if (v.option2 && v.option2.length > 0) {
            if (
              $('.main-product .trigger-option-sw[data-name="option2"]').is(
                "input"
              )
            ) {
              $(
                '.main-product .trigger-option-sw[data-name="option2"][value="' +
                  v.option2 +
                  '"]'
              ).trigger("click");
            } else {
              $('.main-product .trigger-option-sw[data-name="option2"]')
                .val(v.option2)
                .trigger("change");
            }
          }
          if (v.option3 && v.option3.length > 0) {
            if (
              $('.main-product .trigger-option-sw[data-name="option3"]').is(
                "input"
              )
            ) {
              $(
                '.main-product .trigger-option-sw[data-name="option3"][value="' +
                  v.option3 +
                  '"]'
              ).trigger("click");
            } else {
              $('.main-product .trigger-option-sw[data-name="option3"]')
                .val(v.option3)
                .trigger("change");
            }
          }
  ...
  gallery: function () {
    if (window.F1GENZ_vars.product.featured.style_gallery == "style1") {
      // Style 1
      $(
        ".main-product .main-product-feature .main-product-feature-featured"
      ).slick({
        slidesToShow: 1,
        slidesToScroll: 1,
        asNavFor:
      ...
      $(
        ...
        slidesToShow:
          $(
            ".main-product .main-product-feature .main-product-feature-featured a"
          ).length > 5
            ? 4
            : $(
                ".main-product .main-product-feature .main-product-feature-featured a"
  ...
  content: function () {
    $(
      ".main-product-description td, .main-product-description tr, .main-product-description th, .main-product-description thead, .main-product-description tbody"
    ...
    $("body").on(
      "click",
      ".main-product-description .main-product-description-left-head li",
      function () {
        if (
          $(window).scrollTop() > $(".main-product-description").offset().top
        ) {
          $("body, html").animate(
            {
        ...
        $(
          '.main-product-description-left-item[data-type="' + type + '"]'
        ).show();
      }
    );
  },
  sharing: function () {
    $("body").on(
      "click",
      ".main-product .main-product-share .main-product-share-cta",
      function (e) {
        e.preventDefault();
    ...
    $("body").on(
      "click",
      ".main-product .main-product-share .main-product-share-overplay",
      function (e) {
        e.preventDefault();
  ...
  relate: function () {
    if ($(".main-product-relate").length > 0) {
  ...
  together: function () {
    $("body").on(
      "click",
      'button[data-type="main-product-together"]',
      function () {
  ...
  readMore: function () {
    $(
      ".main-product .main-product-description .main-product-description-left-item"
    ).each(function () {
      var checkHeight = $(this)
        .find(".main-product-description-item-data")
        .outerHeight();
      if (checkHeight > 500) {
    ...
    $("body").on(
      "click",
      ".main-product-description .main-product-description-left-item .main-product-description-item-readmore",
      function () {
  ...
  preOrder: function () {
    $("body").on(
      "click",
      'button[data-type="main-product-send-help"]',
      function (e) {
        $("#preOrder-modal").modal();
        $("#preOrder-hard").val(
          $("#main-product-select option:selected").text()
        );
      }
    );
    $("body").on("submit", "#preOrder-modal form", function (e) {
      ...
      $("#preOrder-hard")
        .next()
        .val($("#preOrder-hard").next().val() + " -- " + text);
      F1GENZ.Helper.postForm("#preOrder-modal", "/contact");
    });
  },
};
F1GENZ.Product.init();
